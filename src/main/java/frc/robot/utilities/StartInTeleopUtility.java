...
public class StartInTeleopUtility {
    ...
    public void setRobotPositionBasedOnAlliance() {
        ...
        Alliance alliance = optionalAlliance.get();
            if (alliance == Alliance.Red) {
                ...
                pose = new Pose2d(temp.getX() - 1.33, temp.getY(), temp.getRotation());
            } else {
                temp = m_AprilTagFieldLayout.getTagPose(7).get().toPose2d();//.rotateBy(Rotation2d.fromDegrees(180.0));
                pose = new Pose2d(temp.getX() + 1.33, temp.getY(), temp.getRotation());
            }
    ...
    public void updateStartingPosition() {
        if (!m_HasRunAutonomous && !m_HasSeenTags) {
            System.out.println("updated starting position");
            if(Robot.isReal()) {
